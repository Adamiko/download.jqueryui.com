...
function Builder_1_11_0( build, jqueryUi, components, options ) {
	...
	if ( components.indexOf( "datepicker" ) >= 0 ) {
		...
		build.bundleI18n = Files({
			...
			data: "foobar" /* FIXME rjs({
				prepend: banner( jqueryUi.pkg, files.i18nFiles.paths().map( path.basename ) ),
				files: files,
				include: files.i18nFiles,
				exclude: [ "jquery", "jqueryui/jquery.ui.core", "jqueryui/jquery.ui.datepicker" ]
			})*/
		});
		build.bundleI18nMin = Files({
			path: "jquery-ui-i18n.min.js",
			data: banner( jqueryUi.pkg, files.i18nFiles.map( path.basename ), { minify: true }) // FIXME + stripBanner( files.min( build.bundleI18n[ 0 ] ) )
		});
	} else {
	...
	build.bundleJs = Files({
		path: "jquery-ui.js",
		data: rjs({
			prepend: banner( jqueryUi.pkg, jsComponentFileNames ),
			files: files,
	...
	build.bundleJsMin = Files({
		...
		data: banner( jqueryUi.pkg, jsComponentFileNames, { minify: true } ) /* FIXME + minify build.bundleJs[0].data */
	});
