...
function Builder_1_11_0( build, jqueryUi, components, options ) {
	...
	if ( components.indexOf( "datepicker" ) >= 0 ) {
		...
		build.bundleI18n = Files({
			...
			data: banner( jqueryUi.pkg, files.i18nFiles.paths().map( path.basename ) ) + rjs({
				baseUrl: jqueryUi.path,
	...
	build.bundleJs = Files({
		path: "jquery-ui.js",
		data: banner( jqueryUi.pkg, jsComponentFileNames ) + rjs({
	...
	build.bundleJsMin = Files({
		...
		data: banner( jqueryUi.pkg, jsComponentFileNames, { minify: true } ) /* + minify build.bundleJs[0].data */
	});
